class ItcSimpleSlider {
  ...
  constructor(t, e) {
    ...
    const s = window.getComputedStyle(this._elItems);
    if (
      ((this._delay = Math.round(50 * parseFloat(s.transitionDuration))),
      ...
      this._elListItem.forEach((t, e) => {
        (t.dataset.order = `${e}`),
          (t.dataset.index = `${e}`),
          (t.dataset.translate = "0"),
          this._states.push(0 === e ? 1 : 0);
      }),
      this._config.loop)
    ) {
      const t = this._elListItem.length - 1,
        e = -this._elListItem.length;
      (this._elListItem[t].dataset.order = "-1"),
        (this._elListItem[t].dataset.translate = "" + -this._elListItem.length);
      const s = e * this._clientRect.width;
      this._elListItem[t].style.transform = `translate3D(${s}px, 0px, 0.1px)`;
    }
    this._addIndicators(),
  ...
  _changeActiveItems() {
    this._states.forEach((t, e) => {
      t
        ? this._elListItem[e].classList.add(this.constructor.EL_ITEM_ACTIVE)
        : this._elListItem[e].classList.remove(this.constructor.EL_ITEM_ACTIVE),
        this._elListIndicator.length && t
          ? this._elListIndicator[e].classList.add(
              this.constructor.EL_INDICATOR_ACTIVE
            )
          : this._elListIndicator.length &&
            !t &&
            this._elListIndicator[e].classList.remove(
              this.constructor.EL_INDICATOR_ACTIVE
            );
    }),
      this._el.dispatchEvent(
        new CustomEvent("change.itc.slider", { bubbles: !0 })
      );
  }
  _move() {
    if (
      ...
      const t = this._transform * this._clientRect.width;
      return void (this._elItems.style.transform = `translate3D(${t}px, 0px, 0.1px)`);
    }
    if (!this._config.loop) {
      const t = this._states[0] && "prev" === this._direction,
        e = this._states[this._states.length - 1] && "next" === this._direction;
      if (t || e) return void this._autoplay("stop");
    }
    ...
    const t = this._transform * this._clientRect.width;
    (this._elItems.style.transform = `translate3D(${t}px, 0px, 0.1px)`),
      this._elItems.dispatchEvent(
        new CustomEvent("moving.itc.slider", { bubbles: !0 })
      ),
      this._changeActiveItems(),
      this._isBalancing ||
        ((this._isBalancing = !0),
  ...
  _moveTo(t) {
    const e = this._states.indexOf(1);
    this._direction = t > e ? "next" : "prev";
    for (let s = 0; s < Math.abs(t - e); s++) this._move();
  }
  _autoplay(t) {
  ...
  _addIndicators() {
    if (
      ...
      return;
    let t = "";
    for (let e = 0, { length: s } = this._elListItem; e < s; e++)
      t += `<li class="${this.constructor.EL_INDICATOR}" data-slide-to="${e}"></li>`;
    const e = `<ol class="${this.constructor.EL_INDICATORS}">${t}</ol>`;
    this._el.insertAdjacentHTML("beforeend", e);
  }
  _updateExProperties() {
    const t = Object.values(this._elListItem).map((t) => t),
    ...
    if (this._isBalancing) {
      if ("next" === this._direction) {
        if (
          ...
          (this._exItemMin.style.transform = `translate3D(${t}px, 0px, 0.1px)`),
            this._updateExProperties();
        }
      } else {
        if (
          ...
          const t =
            (this._exTranslateMax - this._elListItem.length) *
            this._clientRect.width;
          (this._exItemMax.style.transform = `translate3D(${t}px, 0px, 0.1px)`),
            this._updateExProperties();
        }
      }
      window.setTimeout(() => {
  ...
  _addEventListener() {
    const t = (t) => {
        if (
          (this._autoplay("stop"),
          t.target.closest(`.${this.constructor.EL_CONTROL}`))
        )
          return;
      ...
      e = (t) => {
        ...
        const e = 0 === t.type.search("touch") ? t.touches[0] : t;
        let s = this._swipeStartPosX - e.clientX;
        const i = this._swipeStartPosY - e.clientY;
        if (!this._hasSwiping) {
          if (Math.abs(i) > Math.abs(s) || 0 === Math.abs(s))
            return void (this._hasSwipeState = !1);
          this._hasSwiping = !0;
        }
        if ((t.preventDefault(), !this._config.loop)) {
          const t = this._states[0] && s <= 0,
            e = this._states[this._states.length - 1] && s >= 0;
          (t || e) && (s /= 4);
        }
        this._elItems.classList.add(this.constructor.TRANSITION_NONE);
        const a = this._transform * this._clientRect.width - s;
      ...
      s = (t) => {
        ...
        let s = this._swipeStartPosX - e.clientX;
        if (0 === s) return void (this._hasSwipeState = !1);
        if (!this._config.loop) {
          const t = this._states[0] && s <= 0,
            e = this._states[this._states.length - 1] && s >= 0;
          (t || e) && (s = 0);
        }
        const i = (s / this._clientRect.width) * 100;
        this._elItems.classList.remove(this.constructor.TRANSITION_NONE),
          i > this.constructor.SWIPE_THRESHOLD
            ? ((this._direction = "next"), this._move())
            : i < -this.constructor.SWIPE_THRESHOLD
    ...
    if (
      (this._el.addEventListener("click", (t) => {
        ...
        if (
          (this._autoplay("stop"),
          e.classList.contains(this.constructor.EL_CONTROL))
        )
          t.preventDefault(), (this._direction = e.dataset.slide), this._move();
        else if (e.dataset.slideTo) {
          t.preventDefault();
          const s = parseInt(e.dataset.slideTo, 10);
          this._moveTo(s);
        }
        this._autoplay();
      }),
      ...
      const i = !!this.constructor.checkSupportPassiveEvents() && {
        passive: !1,
      };
      this._el.addEventListener("touchstart", t, i),
        this._el.addEventListener("touchmove", e, i),
    ...
    if (
      ...
      new ResizeObserver((t) => {
        const { contentRect: e } = t[0];
        if (
          Math.round(10 * this._clientRect.width) === Math.round(10 * e.width)
        )
          return;
        this._clientRect = e;
        const s = e.width * Number(this._elItems.dataset.translate);
        this.reset(s, !0), this._autoplay();
  ...
  reset(t = 0, e = !1) {
    if (
      ...
      this._elItems.classList.add(this.constructor.TRANSITION_NONE),
      (this._elItems.style.transform = `translate3D(${t}px, 0px, 0.1px)`),
      this._elListItem.forEach((t) => {
        const s = e ? Number(t.dataset.translate) * this._clientRect.width : 0;
        t.style.transform = `translate3D(${s}px, 0px, 0.1px)`;
      }),
      !e)
    ) {
      if (
        ...
        const t = this._elListItem.length - 1,
          e = -this._elListItem.length;
        (this._elListItem[t].dataset.order = "-1"),
          (this._elListItem[t].dataset.translate =
            "" + -this._elListItem.length);
        const s = e * this._clientRect.width;
        this._elListItem[t].style.transform = `translate3D(${s}px, 0px, 0.1px)`;
      }
